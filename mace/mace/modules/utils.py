...
def x_to_k_cell(cells):
    ...
    print("bcells:", bcells.shape, "vol:", vol.shape)

    return (bcells, vol[:]) #check: changed the shape of vol. original: vol[:, 0]
...
def get_k_voxel_grid(k_cutoff, delta_k, num_k_rbf):
    ...
    print("k_index_product_set shape if NOT periodic:", k_index_product_set.shape)

    # Orthogonal k-space basis, norm delta_k
    k_cell = torch.tensor(
        [[delta_k, 0, 0], [0, delta_k, 0], [0, 0, delta_k]], dtype=torch.float
    )
    print("k_cell shape if NOT periodic:", k_cell.shape)
